...
var  Dialog = function(){};
  ...
  Dialog.prototype = {
    ...
    createDialogWrap : function( tmpl ){
      ...
      var header = tmpl.header ? 
        '<h4 class="xyDialog_title" id="xyDialogTitle"><a href="javascript:void(0)" title="关闭窗口" class="close_btn" id="closeBtn">&times;</a>' + tmpl.header + '</h4>' :
        '',
        // 确定按钮
        yesBtn = typeof tmpl.yesFn === 'function' ? 
          '<button class="btn_highlight" id="xyDialogYesBtn">' + ( typeof tmpl.yesText === 'string' ? tmpl.yesText : '确定' ) + '</button>' :
          '',
        // 取消按钮  
        noBtn = typeof tmpl.noFn === 'function' || tmpl.noFn === true ? 
          '<button class="btn_normal" id="xyDialogNoBtn">' + ( typeof tmpl.noText === 'string' ? tmpl.noText : '取消' ) + '</button>' :
          '',      
        // footer
        footer = yesBtn === '' && noBtn === '' ? '' :
          '<div class="xyDialog_footer">' + noBtn + yesBtn + '</div>',
        
        dialogTmpl = [
        '<div class="xyDialog_content">',
          header,
          '<div class="xyDialog_text">' + tmpl.content + '</div>',
          footer,
        '</div>'
        ].join(''),

        dialogWrap = doc.getElementById( 'xyDialogWrapper' ),
        rScript = /<[\/]*script[\s\S]*?>/ig;
        
      if( !dialogWrap ){
        dialogWrap = doc.createElement( 'div' );
        dialogWrap.id = 'xyDialogWrapper';
        dialogWrap.className = 'xyDialog_wrapper';
      }
  ...
  var extend = {
    open : function(){
      ...
      // 如果页面中已经缓存弹出层，直接显示
      dialogBox = doc.getElementById( 'xyDialogBox' );
      if( !dialogBox ){
      ...
      var closeBtn = doc.getElementById( 'closeBtn' ),
        dialogTitle = doc.getElementById( 'xyDialogTitle' ),
        dialogYesBtn = doc.getElementById('xyDialogYesBtn'),
        dialogNoBtn = doc.getElementById('xyDialogNoBtn');    
